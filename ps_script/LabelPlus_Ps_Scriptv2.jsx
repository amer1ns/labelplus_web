...
(function (I18n) {
    ...
    I18n.RB_TEMPLATE_NO = "不使用模板（直接新建文件）";
    ...
    I18n.CHECKBOX_NO_LAYER_GROUP = "不对图层进行分组";
    I18n.CHECKBOX_DIALOG_OVERLAY = "启用对话框自动涂白";
...
(function (LabelPlus) {
    ...
    var CustomOptions = /** @class */ (function () {
        function CustomOptions() {
            ...
            this.outputType = OptionOutputType.PSD; // output image file type
            ...
            this.notClose = false; // do not close image document
            this.font = ""; // set font if it is not empty
            ...
            this.actionGroup = ""; // action group name
            this.dialogOverlayLabelGroups = ""; // the label groups need dialog overlay layer, split by ","
            this.dialogOverlayTolerance = 16; // dialog overlay tolerance
        }
...
(function (LabelPlus) {
    ...
    LabelPlus.DEFAULT_LOG_PATH = LabelPlus.APP_DATA_FOLDER + LabelPlus.dirSeparator + "lp_ps_script.log";
    LabelPlus.DEFAULT_INI_PATH = LabelPlus.APP_DATA_FOLDER + LabelPlus.dirSeparator + "lp_ps_script.ini";
    LabelPlus.DEFAULT_DUMP_PATH = LabelPlus.APP_DATA_FOLDER + LabelPlus.dirSeparator + "lp_ps_script.dump";
    LabelPlus.alllog = "";
    LabelPlus.errlog = "";
...
(function (LabelPlus) {
    ...
    function dialogClear(doc, bgLayer, overLayer, labels, tolerance, contract) {
        ...
        for (var i = 0; i < labels.length; i++) {
            ...
            var fill_color = getSelectionColor(doc);
            LabelPlus.log("point " + i + "(" + x + "," + y + ") color=" + fill_color.rgb.hexValue.toString());
            var tmp_layer = doc.artLayers.add();
            ...
            for (var j = 0; j < corners.length; j++) {
                var x_1 = corners[j].x;
                var y_1 = corners[j].y;
                var color = getColor(doc, UnitValue(x_1, 'px'), UnitValue(y_1, 'px'));
                if (color.rgb.hexValue == tmp_color.rgb.hexValue) {
                    LabelPlus.log("detect corner (" + x_1 + "," + y_1 + ") is tmp_color");
                    continue;
                }
                MyAction.magicWand(x_1, y_1, 0, false, true, 'addTo');
            }
            tmp_layer.remove();
            doc.selection.invert();
            if (isSelectionValid()) {
                ...
                LabelPlus.log("selection is not valid, ignore...");
            }
        }
        return true;
    }
...
(function (LabelPlus) {
    ...
    function importImage(img) {
        ...
        if (img.ws.bgLayer && (opts.dialogOverlayLabelGroups !== "")) {
            LabelPlus.log('move "dialog-overlay" before "bg"');
    ...
    function openImageWorkspace(img_filename, template_path) {
        ...
        var pendingDelLayerList = new Array();
        {
            ...
            try {
                dialogOverlayLayer = wsDoc.artLayers.getByName(LabelPlus.TEMPLATE_LAYER.DIALOG_OVERLAY);
            }
            catch (_d) {
                dialogOverlayLayer = wsDoc.artLayers.add();
                dialogOverlayLayer.name = LabelPlus.TEMPLATE_LAYER.DIALOG_OVERLAY;
            }
        }
    ...
    function importFiles(custom_opts) {
        ...
        switch (opts.docTemplate) {
            ...
            case LabelPlus.OptionDocTemplate.Auto:
                ...
                var tempname = app.locale.split("_")[0].toLocaleLowerCase() + ".psd"; // such as "zh_CN" -> zh.psd
                var try_list = [
                    tempdir + tempname,
                    tempdir + "en.psd"
                ];
        ...
        for (var i = 0; i < opts.imageSelected.length; i++) {
            ...
            if (opts.ignoreNoLabelImg && ((_a = lpFile) === null || _a === void 0 ? void 0 : _a.images[orgin_name].length) == 0) { // ignore img with no label
...
(function (LabelPlus) {
    ...
    var LabelPlusInput = /** @class */ (function (_super) {
        ...
        function LabelPlusInput() {
            ...
            _this.lpFile = null; // 儲存解析後的標籤設定檔（.txt或.json）
            // 建立標籤檔案中的圖片名稱與實際圖片檔案的對應關係
            _this.getMatchedFileList = function () {
                var pnl = _this.inputPnl;
                var selectedList = getSelectedItemsText(pnl.chooseImageListBox); // 取得選中的圖片列表
                var fileList = LabelPlus.getImageFilesListOfPath(pnl.sourceTextBox.text); // 取得來源資料夾中的所有圖片檔案
                var replaceImgSuffix = (pnl.replaceImgSuffixCheckBox.value) ? pnl.replaceImgSuffixTextbox.text : ""; // 要替換的副檔名
                var matchImgByOrder = pnl.matchImgByOrderCheckBox.value; // 是否按順序匹配
                var arr = [];
                for (var i = 0; i < selectedList.length; i++) {
                    var filename = selectedList[i].text;
                    var fileindex = selectedList[i].index;
                    if (matchImgByOrder) {
                        // 模式1: 按列表順序匹配（忽略檔名，直接按順序對應）
                        arr.push({
                            file: filename,
            ...
            _this.optPickers = []; // 儲存各個面板的「獲取選項」函式
            // 將選項獲取函式添加到列表中
            ...
            _this.uiLpTextSelect = function (pnl) {
                ...
                pnl.lpTextFileBrowseButton.onClick = function () {
                    ...
                    var fmask = "*.txt;*.json";
                    var f = File.openDialog(I18n.LABEL_TEXT_FILE, fmask); // 開啟檔案選擇對話框
                    // 修改這一行：明確指定桌面為默認路徑
                   // var f = File.openDialog(I18n.LABEL_TEXT_FILE, fmask, new File(Folder.desktop.fsName + "/default.txt"));
                    if (f && f.exists) {
                        pnl.lpTextFileTextBox.text = f.fsName; // 顯示選中的檔案路徑
                        var fl = new Folder(f.path);
                        inputPnl.sourceTextBox.text = fl.fsName; // 自動設定來源路徑
                        outputPnl.targetTextBox.text = fl.fsName + LabelPlus.dirSeparator + 'output'; // 自動設定輸出路徑
                        // 自動偵測常見的圖片資料夾名稱
                    ...
                    _this.lpFile = lpFile;
                    _this.allPanelEnable(true); // 啟用所有面板
                    // 填充圖片列表
                    ...
                    for (var i = 0; i < lpFile.groups.length; i++) {
                        ...
                        inputPnl.chooseGroupListBox[i].selected = true;
                        {
                            ...
                            if (doPnl.groupTextBox.text == "") { // 第一個群組
            ...
            _this.uiSettingsPanel = function (pnl) {
                ...
                var bw = 90;
                var x = offsets[0] - (bw / 2);
                pnl.load = pnl.add('button', [x, y, x + bw, y + 20], I18n.BUTTON_LOAD); // 載入設定按鈕
                x = offsets[1] - (bw / 2);
                pnl.save = pnl.add('button', [x, y, x + bw, y + 20], I18n.BUTTON_SAVE); // 保存設定按鈕
                x = offsets[2] - (bw / 2);
                pnl.reset = pnl.add('button', [x, y, x + bw, y + 20], I18n.BUTTON_RESET); // 重設設定按鈕
                pnl.load.onClick = function () {
                    var def = pnl.defaultFile;
                    var prmpt = pnl.loadPrompt;
                    var sel = Stdlib.createFileSelect(pnl.fileMask);
                    ...
                    var f = Stdlib.selectFileOpen(prmpt, sel, def); // 選擇要載入的設定檔
                    if (f) {
                        _this.opts = readIni(f); // 讀取INI設定檔
                        win.close(4);
                    }
                };
                pnl.save.onClick = function () {
                    ...
                    var f = Stdlib.selectFileSave(prmpt, sel, def); // 選擇保存設定檔的位置
                    if (f) {
                        var mgr = win.mgr;
                        var res = mgr.validatePanel(win.appPnl, win.ini, true);
                        if (typeof (res) != 'boolean') {
                            writeIni(f, res); // 寫入INI設定檔
                        }
                    }
                };
                pnl.reset.onClick = function () {
                    _this.opts = new LabelPlus.CustomOptions(); // 重設為預設選項
                    _this.lpFile = null;
                    win.close(4);
                };
                return {};
            };
            // 輸入設定面板UI：設定圖片來源和選擇要處理的內容
            _this.uiInputPanel = function (pnl) {
                ...
                pnl.sourceBrowse.onClick = function () {
                    try {
                        ...
                        var f = Stdlib.selectFolder(I18n.LABEL_SOURCE, def); // 選擇來源資料夾
                        if (f) {
                ...
                pnl.matchImgByOrderCheckBox.onClick = function () {
                    if (pnl.matchImgByOrderCheckBox.value) {
                        pnl.replaceImgSuffixCheckBox.value = false; // 與"替換副檔名"互斥
                ...
                pnl.checkSourceMatchButton.onClick = function () {
                    var matchList = _this.getMatchedFileList(); // 取得匹配結果
                ...
                pnl.replaceImgSuffixCheckBox.onClick = function () {
                    if (pnl.replaceImgSuffixCheckBox.value) {
                        pnl.matchImgByOrderCheckBox.value = false; // 與"按順序匹配"互斥
                ...
                var getOption = function (opts, toFile) {
                    if (!toFile) {
                        ...
                        opts.imageSelected = _this.getMatchedFileList(); // 取得選擇的圖片匹配列表
                        ...
                        for (var i = 0; i < pnl.chooseGroupListBox.selection.length; i++) {
                            opts.groupSelected[i] = pnl.chooseGroupListBox.selection[i].text; // 取得選擇的群組
                        }
                    }
                    return opts;
                };
            ...
            _this.uiOutputPanel = function (pnl) {
                ...
                pnl.targetBrowse.onClick = function () {
                    try {
                        ...
                        f = Stdlib.selectFolder(I18n.LABEL_TARGET, def); // 選擇輸出資料夾
                        if (f) {
                ...
                pnl.ignoreNoLabelImgCheckBox.value = true; // 預設忽略無標籤圖片
                xx += 250;
            ...
            _this.uiStylePanel = function (pnl) {
                ...
                xxx += 200;
                pnll.autoTemplateRb.value = true; // 預設自動選擇模板
                ...
                xxx += 135;
                pnll.customTemplateTextbox = pnl.add('edittext', [xxx, yyy, xxx + 180, yyy + 20]);
                xxx += 185;
                pnll.customTemplateTextButton = pnl.add('button', [xxx, yyy - 2, xxx + 30, yyy + 20], '...');
                xxx += 30;
                ...
                pnll.customTemplateTextButton.onClick = function () {
                    try {
                        ...
                        var f = Stdlib.selectFileOpen(I18n.RB_TEMPLATE_CUSTOM, "*.psd;*.tif;*.tiff", def); // 選擇模板檔案
                        if (f)
                ...
                pnl.textDirList.selection = pnl.textDirList.find(I18n.LIST_TEXT_DIT_ITEMS[0]); // 預設第一個選項
                xx = xOfs;
                yy += 23;
                {
                    // 字型設定
                    ...
                    _this.createFontPanel(pnl.font); // 建立字型選擇面板
                    pnl.font.label.text = " ";
                    pnl.font.family.enabled = false;
                    pnl.font.style.enabled = false;
                    pnl.font.fontSize.enabled = false;
                    pnl.font.family.selection = pnl.font.family.find("SimSun"); // 預設字型為SimSun
                    xx = xOfs;
                    yy += 25;
                }
                // 行距設定
                ...
                pnl.textLeadingTextBox.text = "120"; // 預設行距120%
                xx += 55;
                ...
                if (opts.font !== undefined) {
                    if (opts.font === "") {
                        ...
                        try {
                            pnl.font.setFont(opts.font, opts.fontSize); // 設定字型
                        }
                        catch (e) {
            ...
            _this.uiAutomationPanel = function (pnl) {
                ...
                xx += 260;
                var ary = Stdlib.getActionSets(); // 取得可用的動作集
                ...
                pnl.overlayPnl = pnl.add('panel', [xx, yy, xx + 460, yy + 75]);
                {
                    ...
                    doPnl_1.toleranceTextBox.text = "16"; // 預設容差16
                    xx_1 = xOfs;
                    yy_1 += 20;
                ...
                if (opts.textReplace !== undefined) {
                    ...
                    pnl.textReplaceTextBox.text = (opts.textReplace !== "") ? opts.textReplace : "！？->!?|...->…"; // 預設文字替換規則
                ...
                var getOption = function (opts) {
                    ...
                    if (pnl.runActionGroupCheckBox.value && pnl.runActionGroupList.selection) {
                        opts.actionGroup = pnl.runActionGroupList.selection.text; // 取得選擇的動作組
                    }
                    ...
                    if (pnl.overlayPnl.toleranceTextBox.text !== "") {
                        opts.dialogOverlayTolerance = pnl.overlayPnl.toleranceTextBox.text; // 取得容差值
                    }
                    return opts;
                };
                return { getOption: getOption };
            ...
            _this.mainPannel = function (pnl) {
                ...
                _this.allPanelEnable(_this.lpFile != null); // 根據是否有載入標籤檔案來啟用/禁用面板
                return pnl;
            };
            // 統整所有面板的選項
            _this.geCustomOptions = function (toFile) {
                ...
                for (var i = 0; i < _this.optPickers.length; i++) {
                    var ret = _this.optPickers[i](new_opts, toFile); // 呼叫每個面板的選項獲取函式
                    if (ret == null) {
                        return null; // 如果有任何面板返回null，則整體返回null
                    }
                    new_opts = ret;
                }
                return new_opts;
            ...
            // 嘗試載入自動保存的設定檔
            try {
                _this.opts = readIni(LabelPlus.DEFAULT_INI_PATH);
    ...
    LabelPlusInput.prototype.validatePanel = function (pnl, ini, tofile) {
        var opts = this.geCustomOptions(tofile); // 統整所有選項
        if (opts == null) {
            return true; // 驗證失敗，但不關閉視窗
        }
        // 檢查每個選擇的圖片是否都有對應的實際檔案
        for (var i = 0; i < opts.imageSelected.length; i++) {
            ...
            if (!LabelPlus.FileIsExists(opts.source + LabelPlus.dirSeparator + item.matched_file)) {
                ...
                LabelPlus.Emit(this.inputPnl.checkSourceMatchButton.onClick); // 觸發匹配檢查
                return true; // 驗證失敗，但不關閉視窗
            }
        }
        return opts; // 驗證成功，返回選項物件進行處理
    };
    // 主要處理函式：執行批次處理
    LabelPlusInput.prototype.process = function (opts, doc) {
        var result = false;
        try {
            writeIni(LabelPlus.DEFAULT_INI_PATH, opts); // 自動保存設定
            result = LabelPlus.importFiles(opts); // 執行主要的批次處理
        }
        catch (e) {
            // 錯誤處理：記錄錯誤日誌
        ...
        if (result && (LabelPlus.errlog == "")) {
            alert(I18n.COMPLETE); // 完全成功
            return;
        }
        else if (result && (LabelPlus.errlog != "")) {
            alert(I18n.COMPLETE_WITH_ERROR, "error", true); // 成功但有錯誤
        }
        else if (!result) {
            alert(I18n.COMPLETE_FAILED, "error", true); // 處理失敗
        }
        // 顯示錯誤日誌視窗
    ...
    function readIni(iniFile) {
        ...
        if (file.exists && file.open("r", "TEXT", "????")) {
            ...
            file.encoding = 'UTF-8';
            var str = file.read();
            ini = jamJSON.parse(str); // 解析JSON字串為物件
            file.close();
        }
        return ini;
    }
